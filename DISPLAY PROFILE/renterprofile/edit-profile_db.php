...
// Fetch user data
$sql = "SELECT * FROM users WHERE id = 1"; // Change the ID as needed
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE users SET name=?, school=?, school_address=?, phone=?, address=?, email=?, facebook=?, instagram=?, status=?, profile_image=? WHERE id=1");
